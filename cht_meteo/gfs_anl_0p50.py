"""GFS 0.5-degree analysis dataset downloaded from the NCEI THREDDS server."""
...
class MeteoDatasetGFSAnalysis0p50(MeteoDataset):
    """GFS 0.5-degree analysis dataset.

    Downloads analysis data from the NCEI THREDDS Data Server via OPeNDAP.
    ...
    **kwargs
    ...
    def download_analysis_times(self, requested_times: list, **kwargs) -> None:
        """Download GFS analysis fields for the specified time steps.

        Parameters
        ----------
        requested_times : list of datetime
            Individual timestamps to download (3-hourly).
        **kwargs
        ...
        for it, time in enumerate(requested_times):
            try:
                ...
                if time < toldnew:
                    ...
                    url = f"{base_url}{month_string}/{date_string}/"
                    name = f"gfsanl_4_{date_string}_{cstr}.grb2"
                else:
                    base_url = "https://www.ncei.noaa.gov/thredds/dodsC/model-gfs-g4-anl-files/"
                    url = f"{base_url}{month_string}/{date_string}/"
                    name = f"gfs_4_{date_string}_{cstr}.grb2"
                ...
                print(e)
                # Try another time
                print(f"Could not read {full_url} !")
        ...
        for it, time in enumerate(requested_times):
            ...
            for ind, param in enumerate(param_list):
                if param == "precipitation":
                    if time < toldnew:
                        name = f"gfsanl_4_{date_string}_{crstr}.grb2"
                    else:
                        name = f"gfs_4_{date_string}_{crstr}.grb2"
                ...
                try:
                    ...
                    for iattempt in range(10):
                        try:
                            ...
                            if iattempt > 0:
                                print(f"Success at attempt no {iattempt + 1}")
                            break
                        except Exception:
                            # Try again
                            pass
            ...
            ds.to_netcdf(
                path=os.path.join(
                    self.path, f"{self.name}.{time.strftime('%Y%m%d_%H%M')}.nc"
                )
            )
            ds.close()
