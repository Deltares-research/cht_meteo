...
class MetGet:
    def __init__(
        ...
        if config_path:
            ...
            if "priority_storm" in config:
                if config["priority_storm"] == "tc_vitals":
                    ...
                    if not self.priority_storm:
                        ...
                        print(
                            f"Priority storm {self.priority_storm} found from https://ftp.nhc.noaa.gov/atcf/com/tcvitals"
                        )
                else:
                    self.priority_storm = config["priority_storm"]
                    print(
                        f"Priority storm {self.priority_storm} provided in config file."
                    )
            else:
            ...
            raise ValueError(
                "Either 'config_path' or both 'apikey' and 'endpoint' must be provided"
            )

        self.meteo_variables = {}
        self.meteo_variables["wind"] = MeteoVariable(
            unit="m/s", fill_value=0.0, metget_name="wind_pressure"
        )
        self.meteo_variables["barometric_pressure"] = MeteoVariable(
            unit="Pa", fill_value=102000.0, metget_name="wind_pressure"
        )
        self.meteo_variables["precipitation"] = MeteoVariable(
            unit="kg.m-2.hour-1", fill_value=0.0, metget_name="rain"
        )

    def get_forecasts(
        self, model: str = "coamps", start: datetime = None, end: datetime = None
    ) -> dict:
        """
    ...
    def get_coamps_available_storms(
        self, time_range: Tuple[datetime.datetime, datetime.datetime], dt: int = 1
    ):
        # Get available forecast for the time_range
        coamps_forecasts = self.get_forecasts(
            model="coamps", start=time_range[0], end=time_range[1]
        )

        # Create time-series based on time_range and dt
        requested_times = (
            pd.date_range(start=time_range[0], end=time_range[1], freq=f"{dt}h")
            .to_pydatetime()
            .tolist()
        )
        requested_times = [
    ...
    def get_coamps_quasi_hindcast(
        ...
        for ti in df.index:
            ...
            if l_storms_only:
                storms = [
                    name
                    for name in df.loc[ti, "storms"]
                    if int(name.split("L")[0]) < 90
                ]
            else:
                storms = df.loc[ti, "storms"]
            if len(storms) > 0:
                if (
                    priority_storm in storms
        ...
        for ti in df.index:
            if df.loc[ti, "storm_id"] is not None:
                cycle = cycles[df.loc[ti, "storm_id"]][
                    cycles[df.loc[ti, "storm_id"]] <= ti
                ][-1]
    ...
    def get_meteo(
        ...
        if status_code != 200:
            raise RuntimeError(
                f"Unexpected status code: {status_code}, when downloading data."
            )
